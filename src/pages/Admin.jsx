...
import { Calendar, MessageSquare, Users, Briefcase, Settings, LogOut, Plus, Trash2, Edit3, Save, X, Star } from 'lucide-react';
  ...
  return (
    <main className="admin-page">
      <aside className="admin-sidebar">
        <div className="admin-sidebar-header"><Star size={20} fill="currentColor" className="text-brand" /><span>EBC Admin</span></div>
      ...
      <div className="admin-content">
        ...
        {tab==='events' && <CRUD filename="events.json" label="Event" fields={[
          {key:'name',label:'Event Name',type:'text'},{key:'description',label:'Description',type:'textarea'},
          {key:'date',label:'Date',type:'date'},{key:'time',label:'Time',type:'text'},
          {key:'venue',label:'Venue',type:'text'},{key:'mapsLink',label:'Maps Link',type:'url'},
        ...
        {tab==='stories' && <CRUD filename="stories.json" label="Story" fields={[
          {key:'name',label:'Name',type:'text'},{key:'designation',label:'Designation',type:'text'},
          {key:'story',label:'Story',type:'textarea'}
        ]}/>}
        {tab==='groups' && <CRUD filename="whatsapp-groups.json" label="Group" fields={[
          {key:'name',label:'Group Name',type:'text'},{key:'area',label:'Area',type:'text'},
          {key:'description',label:'Description',type:'textarea'},{key:'whatsappLink',label:'WhatsApp Link',type:'url'},
          {key:'active',label:'Active',type:'checkbox'}
        ]}/>}
        {tab==='opportunities' && <CRUD filename="opportunities.json" label="Role" fields={[
          {key:'role',label:'Role Name',type:'text'},{key:'description',label:'Description',type:'textarea'},
          {key:'timeCommitment',label:'Time Commitment',type:'text'},{key:'skills',label:'Skills (comma separated)',type:'text'},
          {key:'formLink',label:'Form Link',type:'url'},{key:'active',label:'Active',type:'checkbox'}
        ]}/>}
...
function CRUD({filename,label,fields}) {
  const {data,saveData} = useJsonData(filename);
  ...
  const startAdd = () => {
    const n={id:`${Date.now()}`};
    fields.forEach(f=>{n[f.key]=f.type==='checkbox'?true:''});
    setForm(n); setAdding(true); setEditId(null);
  };
  ...
  const save = () => {
    ...
    if(adding) saveData([...items,p]); else saveData(items.map(i=>i.id===editId?p:i));
    setEditId(null); setAdding(false); setForm({});
  };
  const del = (id) => { if(window.confirm(`Delete?`)) saveData(items.filter(i=>i.id!==id)); };
          ...
          <div className="admin-form-grid">
            {fields.map(f=>(
              <div key={f.key} className={`form-group ${f.type==='textarea'?'form-group-full':''}`}>
                ...
                {f.type==='textarea'?<textarea className="form-input form-textarea" value={form[f.key]||''} onChange={e=>setForm({...form,[f.key]:e.target.value})} rows={3}/>
                :f.type==='select'?<select className="form-input form-select" value={form[f.key]||''} onChange={e=>setForm({...form,[f.key]:e.target.value})}><option value="">Select</option>{f.options.map(o=><option key={o} value={o}>{o}</option>)}</select>
                :f.type==='checkbox'?<label className="admin-checkbox"><input type="checkbox" checked={form[f.key]||false} onChange={e=>setForm({...form,[f.key]:e.target.checked})}/><span>Enabled</span></label>
                :<input type={f.type} className="form-input" value={form[f.key]||''} onChange={e=>setForm({...form,[f.key]:e.target.value})}/>}
              </div>
            ))}
          </div>
          <div className="admin-form-actions">
            <button className="btn btn-primary btn-sm" onClick={save}><Save size={16}/> Save</button>
            <button className="btn btn-ghost btn-sm" onClick={()=>{setEditId(null);setAdding(false);}}><X size={16}/> Cancel</button>
          </div>
        </div>
      )}
...
function SiteSettings() {
  const {data:settings,saveData} = useSettings();
  ...
  return (
    <div>
      ...
      <div className="admin-edit-form">
        ...
        <div className="admin-form-actions">
          <button className="btn btn-primary btn-md" onClick={()=>{saveData(form);alert('Saved!');}}><Save size={16}/> Save Settings</button>
        </div>
      </div>
    </div>
  );
